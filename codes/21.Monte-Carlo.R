...
## Parameters and seed
beta_1 = -0.005 # estimated elasticity - here it is directly the regression coefficient
set.seed(1)  # Seed
n = 1000     # Sample size
...
for (i in 1:M){ #  M is the number of iterations
  ...
  X_i = rnorm(n, mean = -0.005, sd = 0.00847) # mean and sd of the parameter
  X_i = beta_1 + U_i  # Dependent variable
  
  # Formulate data.table
  data_i = data.table(X = X_i)
  
  # Run regressions
  ols_i <- lm(data = data_i, X ~ 1)
