...
library(xtable)
print(xtable(df_table, type = "latex"), file = "./results/Tables/Benefits_table.tex")

##########################################################################

cda <- st_read("./Census/CAN/can.shp")

csd <- st_read("./Census/CEN/CAN/lcsd000b21a_e.shp")
...
df_all <- df_sim_final%>%
  group_by(prov)%>%
  #subset(prov == "QC")%>%
  select(GeoUID,val_wf,val_nwf,val_tot,sim_id,elast_250m,elast_750m,prov,bfp_500,bfp_1000)%>%
  mutate(bfp_all = bfp_500+bfp_1000)%>%
  group_by(GeoUID)%>%
  mutate(ave_tot = mean(val_tot))%>%
...
df_all$GeoUID <- as.character(df_all$GeoUID)

cda_map <- cda%>%
  left_join(df_all)

st_write(cda_map, "./Census/benefitmap_cda.shp", delete_dsn = T)
...
csd_map <- cda_map%>%
  ...
  group_by(CSD_UID)%>%
  mutate(ave_tot = sum(ave_tot))%>%
  mutate(ave_bfp = sum(bfp_all))%>%
  mutate(val_pphh = ave_tot/ave_bfp)%>%
  select(CSD_UID,ave_tot,val_pphh,prov,ave_bfp,bfp_all, CD_UID)%>%
  distinct(CSD_UID, .keep_all = T)%>%
  rename("CSDUID"="CSD_UID")

t <- c("10", "11", "12", "13", "24", "35", "46", "47", "48", "59")
...
st_write(csd_map, "./Census/benefitmap_csd.shp", delete_dsn = T)



prov <- st_read("./Census/CEN/CAN/lpr_000b21a_e/lpr_000b21a_e.shp")


prov <- prov%>%
  filter(PRUID %in% t)

st_write(prov, "./Census/prov.shp", delete_dsn = T)
...
cd_map <- csd_map%>%
  ...
  mutate(ave_bfp = sum(bfp_all))%>%
  mutate(val_pphh = ave_tot/ave_bfp)%>%
  distinct(CD_UID, .keep_all = T)%>%
  select(-CSDUID)


t <- c("10", "11", "12", "13", "24", "35", "46", "47", "48", "59")

ccd <- st_read("./Census/CEN/CAN/lcd_000b21a_e/lcd_000b21a_e.shp")%>%
  rename("CD_UID"="CDUID")%>%
  left_join(cd_map)%>%
  filter(PRUID %in% t)
...
st_write(ccd, "./Census/benefitmap_ccd.shp", delete_dsn = T)
...
cda_map <- cda_map%>%
  mutate(val_tot_in_1000 = val_tot/1000)


p <- cda_map%>%
  ggplot( aes(x=prov, y=val_tot_in_1000, fill=prov)) +
  ...
  ggtitle("") +
  xlab("Province") +
  ylab("Total value within a CDA in 1000's ($)")
...
max(cda_map$val_tot, na.rm = T)
min(cda_map$val_tot, na.rm = T)
